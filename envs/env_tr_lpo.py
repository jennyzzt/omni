...
class Env(env_tr_uni.Env):
  ...
  def __init__(
    ...
    self.p_fast, self.p_slow = None, None
  ...
  def push_info(self, info):
    ...
    self.p_fast = self.eval_tsr
    ema_alpha, p_theta = 0.1, 0.1
    self.p_slow = self.p_fast * ema_alpha + self.p_slow * (1 - ema_alpha) if self.p_slow is not None else self.p_fast
    p_fast_reweigh = ((1 - p_theta) * self.p_fast) / (self.p_fast + p_theta * (1 - 2 * self.p_fast))
    p_slow_reweigh = ((1 - p_theta) * self.p_slow) / (self.p_slow + p_theta * (1 - 2 * self.p_slow))
    self.learning_progress = np.abs(p_fast_reweigh - p_slow_reweigh)
